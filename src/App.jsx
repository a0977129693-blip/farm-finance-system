      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-8 space-y-8">
        ...
        <Card className="bg-emerald-50 border-2 border-dashed border-emerald-300 p-8 shadow-inner">
          <Flex className="mb-4">
            ...
            <Button variant="secondary" color="emerald" onClick={generateDiagnosis} loading={diagLoading}>
              重新生成報告
            </Button>
          </Flex>
          {diagnosis ? (
            <div className="bg-white p-6 rounded-2xl border-2 border-emerald-100 text-emerald-900 leading-relaxed shadow-sm font-medium">
              {diagnosis}
            </div>
          ) : (
                ...
                <div key={r.id} className="p-5 flex justify-between items-center hover:bg-slate-50 transition-colors">
                  {editingId === r.id ? (
                    <div className="flex gap-2 flex-1 items-center animate-pulse">
                      <input className="border-2 border-blue-500 p-2 rounded w-24" type="number" value={editForm.amount} onChange={e => setEditForm({...editForm, amount: Number(e.target.value)})} />
                      <input className="border-2 border-blue-500 p-2 rounded flex-1" value={editForm.description} onChange={e => setEditForm({...editForm, description: e.target.value})} />
